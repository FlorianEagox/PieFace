...
//
//            }
            mTickAndCirclePaint.textSize = mCalendar.get(Calendar.SECOND).toFloat()
//            canvas.drawText("${mCalendar.get(Calendar.HOUR)}:${mCalendar.get(Calendar.MINUTE)}:${mCalendar.get(Calendar.SECOND)}",
//                mCenterX, mCenterY, mTickAndCirclePaint)
            mTickAndCirclePaint.style = Paint.Style.FILL
            val arcAngle = 360f * (mCalendar.get(Calendar.SECOND) / 60f)
            canvas.drawArc(
                0f, 0f, canvas.width.toFloat(),
                canvas.height.toFloat(), 0f, arcAngle, true, mTickAndCirclePaint
            )
